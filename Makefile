...
ifeq ($(MAKECMDGOALS),ipq)
	...
	CROSS_LIBPATH:=-L$(TOOLCHAIN_BUILD_DIR)/uClibc-0.9.33.2/lib -L$(TOOLCHAIN_DIR)/lib -L$(TARGET_BUILD_DIR)/lib -L$(TARGET_BUILD_DIR)/usr/lib -L$(TARGET_BUILD_DIR)/root-ipq806x/lib -L$(TARGET_BUILD_DIR)/root-ipq806x/usr/lib -L$(TARGET_DIR)/root-ipq806x/lib -L$(TARGET_DIR)/usr/lib
endif
CXX := $(CROSS_COMPILE)g++
